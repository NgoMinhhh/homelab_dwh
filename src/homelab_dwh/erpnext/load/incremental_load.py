...
@task(name="erpenxt-incremental-load")
def load_inrecemental_database() -> str:
...
@flow(name="erpnext-incremental-load",log_prints=True)
def load_incremental_database_flow() -> None:
    logger = get_run_logger()
    logger.info("Starting ERPNext incremental load into Postgres DWH")
    result = load_inrecemental_database()
    logger.info(f"Load finished: {result} ")

if __name__ == "__main__":
    # Load all tables from the database.
    # Warning: The sample database is very large
    load_incremental_database_flow()
